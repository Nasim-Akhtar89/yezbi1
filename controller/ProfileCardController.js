...
module.exports.storeLink = async function (req, res) {
  ...
  try {
    ...
    for (let i = 0; i < usrProfileCard.links.length; i++) {
      if (usrProfileCard.links[i].linkValue === linkValue) {
    ...
    res.status(500).json({
      linkAdded: false,
      links: null,
      error: err.message != "" ? err.message : "Failed to save new link! possible reasons: Invalid email OR empty linkType/linkValue...",
    });
  }
};

module.exports.updateLink = async function (req, res) {
  let { email = "", linkName = "", linkValue = "", linkId = "", isBusiness = false } = req.body;
  ...
  try {
    ...
    let currElLink = usrProfileCard.links.id(linkId),
...
module.exports.updateEditProfileObselete = async function (req, res) {
  let { email = "", name = "", location = "", bio = "", businessClient = false, theme = "" } = req.body;
  ...
  try {
    ...
    if (usrProfileCard.name === name) {
      name = "";
    }
    if (usrProfileCard.location === location) {
      location = "";
    }
    if (usrProfileCard.bio === bio) {
      bio = "";
    }
    if (usrProfileCard.theme === theme) {
      theme = "";
    }
    ...
    if (!!name) {
      usrProfileCard.name = name;
    }
    if (!!location) {
      usrProfileCard.location = location;
    }
    if (!!bio) {
      usrProfileCard.bio = bio;
    }
    if (!!theme) {
      usrProfileCard.theme = theme;
    }
...
module.exports.updateEditProfile = async function (req, res) {
  ...
  try {
    ...
    res.status(500).json({
      ...
      data: null,
      error:
        "Failed to save Edit Details! possible reasons: Invalid email OR No user found OR error during file handling (Wrong mimetype OR Save operation failed. allowed mimetype " +
        imageFormats.join(", ") +
        ").",
    });
  }
...
module.exports.togglePrivateMode = async function (req, res) {
  ...
  try {
    ...
    // const privateMode = usrProfileCard.private;
    // usrProfileCard.private = !privateMode;
    // await usrProfileCard.save();
    res.status(200).json({
